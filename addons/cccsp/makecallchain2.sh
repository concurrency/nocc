...
TMPFILE=/tmp/gencallchain2.tmp.$$
TMPFILE2=/tmp/gencallchain2.tmp2.$$
...
cat $TMPFILE | while [ : ]; do
	...
	if [ $INCSIZE -eq 1 ]; then
		STKBYTES=$(cat *-ev3.su | grep -w $FNAME | awk '{print($2);}' | head -1)
		STKQUAL=$(cat *-ev3.su | grep -w $FNAME | awk '{print($3);}' | head -1)
