/*
 *	oil_parser.c -- oil parser for nocc
...
/*{{{  forward decls*/
static int oil_parser_init (lexfile_t *lf);
static void oil_parser_shutdown (lexfile_t *lf);
static tnode_t *oil_parser_parse (lexfile_t *lf);
static tnode_t *oil_parser_descparse (lexfile_t *lf);
static int oil_parser_prescope (tnode_t **tptr, prescope_t *ps);
static int oil_parser_scope (tnode_t **tptr, scope_t *ss);
static int oil_parser_typecheck (tnode_t *tptr, typecheck_t *tc);
static int oil_parser_typeresolve (tnode_t **tptr, typecheck_t *tc);
static int oil_parser_fetrans (tnode_t **tptr, fetrans_t *fe);
static char **oil_getlanglibs (target_t *target, int src);

static tnode_t *oil_parser_parseproc (lexfile_t *lf);
static tnode_t *oil_parser_parsedef (lexfile_t *lf);
static tnode_t *oil_declorproc (lexfile_t *lf);
static tnode_t *oil_indented_declorproc_list (lexfile_t *lf);
static tnode_t *oil_indented_dguard_list (lexfile_t *lf);
static tnode_t *oil_indented_tcase_list (lexfile_t *lf);
static tnode_t *oil_indented_exprproc_list (lexfile_t *lf);


/*}}}*/
/*{{{  global vars*/

oil_pset_t oil;		/* attach tags, etc. here */

langparser_t oil_parser = {
	.langname =		"oil",
	.init =			oil_parser_init,
	.shutdown =		oil_parser_shutdown,
	.parse =		oil_parser_parse,
	.descparse =		oil_parser_descparse,
	.prescope =		oil_parser_prescope,
	.scope =		oil_parser_scope,
	.typecheck =		oil_parser_typecheck,
	.typeresolve =		oil_parser_typeresolve,
	.postcheck =		NULL,
	.fetrans =		oil_parser_fetrans,
	.getlangdef =		oil_getlangdef,
	.getlanglibs =		oil_getlanglibs,
	.maketemp =		NULL,
	.makeseqassign =	NULL,
	.makeseqany =		NULL,
	.tagstruct_hook =	(void *)&oil,
	.lexer =		NULL
};
...
static oil_parse_t *oil_priv = NULL;
...
/*{{{  static int oil_opthandler_stopat (cmd_option_t *opt, char ***argwalk, int *argleft)*/
/*
 *	option handler for oil "stop" options
 */
static int oil_opthandler_stopat (cmd_option_t *opt, char ***argwalk, int *argleft)
{
	compopts.stoppoint = (int)((uint64_t)opt->arg);
#if 0
fprintf (stderr, "oil_opthandler_stopat(): setting stop point to %d\n", compopts.stoppoint);
#endif
	return 0;
}
/*}}}*/
...
oil_fetrans1_t *oil_newfetrans1 (void)
{
	oil_fetrans1_t *fe1 = (oil_fetrans1_t *)smalloc (sizeof (oil_fetrans1_t));
...
/*}}}*/
/*{{{  void oil_freefetrans1 (oil_fetrans1_t *fe1)*/
/*
 *	frees a oil_fetrans1_t structure
 */
void oil_freefetrans1 (oil_fetrans1_t *fe1)
{
	if (!fe1) {
		nocc_serious ("oil_freefetrans1(): NULL pointer!");
		return;
	}
...
/*}}}*/
/*{{{  oil_fetrans15_t *oil_newfetrans15 (void)*/
/*
 *	creates a new oil_fetrans15_t structure
 */
oil_fetrans15_t *oil_newfetrans15 (void)
{
	oil_fetrans15_t *fe15 = (oil_fetrans15_t *)smalloc (sizeof (oil_fetrans15_t));
...
/*}}}*/
/*{{{  void oil_freefetrans15 (oil_fetrans15_t *fe15)*/
/*
 *	frees a oil_fetrans15_t structure
 */
void oil_freefetrans15 (oil_fetrans15_t *fe15)
{
	if (!fe15) {
		nocc_serious ("oil_freefetrans15(): NULL pointer!");
		return;
	}
	sfree (fe15);
	return;
...
/*}}}*/
/*{{{  oil_fetrans2_t *oil_newfetrans2 (void)*/
/*
 *	creates a new oil_fetrans2_t structure
 */
oil_fetrans2_t *oil_newfetrans2 (void)
{
	oil_fetrans2_t *fe2 = (oil_fetrans2_t *)smalloc (sizeof (oil_fetrans2_t));
...
/*}}}*/
/*{{{  void oil_freefetrans2 (oil_fetrans2_t *fe2)*/
/*
 *	frees a oil_fetrans2_t structure
 */
void oil_freefetrans2 (oil_fetrans2_t *fe2)
{
	if (!fe2) {
		nocc_serious ("oil_freefetrans2(): NULL pointer!");
		return;
	}
	sfree (fe2);
	return;
...
/*}}}*/
/*{{{  oil_fetrans3_t *oil_newfetrans3 (void)*/
/*
 *	creates a new oil_fetrans3_t structure
 */
oil_fetrans3_t *oil_newfetrans3 (void)
{
	oil_fetrans3_t *fe3 = (oil_fetrans3_t *)smalloc (sizeof (oil_fetrans3_t));
...
/*}}}*/
/*{{{  void oil_freefetrans3 (oil_fetrans3_t *fe3)*/
/*
 *	frees a oil_fetrans3_t structure
 */
void oil_freefetrans3 (oil_fetrans3_t *fe3)
{
	if (!fe3) {
		nocc_serious ("oil_freefetrans3(): NULL pointer!");
		return;
	}
	sfree (fe3);
	return;
...
static oil_parse_t *oil_newoilparse (void)
{
	oil_parse_t *gpse = (oil_parse_t *)smalloc (sizeof (oil_parse_t));
...
/*}}}*/
/*{{{  static void oil_freeoilparse (oil_parse_t *gpse)*/
/*
 *	frees an oil_parse_t structure
 */
static void oil_freeoilparse (oil_parse_t *gpse)
{
	if (!gpse) {
		nocc_warning ("oil_freeoilparse(): NULL pointer!");
		return;
	}
	if (gpse->langdefs) {
...
/*}}}*/
/*{{{  oil reductions*/
/*{{{  void *oil_nametoken_to_hook (void *ntok)*/
/*
...
/*}}}*/
/*{{{  static void oil_reduce_checkfixio (dfastate_t *dfast, parsepriv_t *pp, void *rarg)*/
/*
...
#if 0
fhandle_printf (FHAN_STDERR, "oil_reduce_checkfixio(): pp=%p, pp->lf=%p\n", pp, pp->lf);
#endif
	if (snode->tag == oil.tag_MARKEDIN) {
		token_t *tok = lexer_newtoken (SYMBOL, "?");

		snode = tnode_nthsubof (snode, 0);
		tok->origin = pp->lf;
		lexer_pushback (pp->lf, tok);
	} else if (snode->tag == oil.tag_MARKEDOUT) {
		token_t *tok = lexer_newtoken (SYMBOL, "!");
...
static int declify_modprewalk (tnode_t **tptr, void *arg)
{
	oil_declify_t *gdl = (oil_declify_t *)arg;
	int i = 1;
...
static int autoseq_modprewalk (tnode_t **tptr, void *arg)
{
	oil_autoseq_t *gas = (oil_autoseq_t *)arg;
	int i = 1;
...
static int fetrans1_modprewalk (tnode_t **tptr, void *arg)
{
	oil_fetrans1_t *fe1 = (oil_fetrans1_t *)arg;
	int i = 1;
...
static int fetrans15_modprewalk (tnode_t **tptr, void *arg)
{
	oil_fetrans15_t *fe15 = (oil_fetrans15_t *)arg;
	int i = 1;
...
static int fetrans2_modprewalk (tnode_t **tptr, void *arg)
{
	oil_fetrans2_t *fe2 = (oil_fetrans2_t *)arg;
	int i = 1;
...
static int fetrans3_modprewalk (tnode_t **tptr, void *arg)
{
	oil_fetrans3_t *fe3 = (oil_fetrans3_t *)arg;
	int i = 1;
...
/*}}}*/
/*{{{  langdef_t *oil_getlangdef (void)*/
/*
 *	returns the language definition for oil, or NULL if none
 */
langdef_t *oil_getlangdef (void)
{
	if (!oil_priv) {
	...
	return oil_priv->langdefs;
}
/*}}}*/
/*{{{  static char **oil_getlanglibs (target_t *target, int src)*/
/*
 *	returns the language libraries for oil, or NULL if none
...
static char **oil_getlanglibs (target_t *target, int src)
{
	...
	if (cccsp_get_subtarget () == CCCSP_SUBTARGET_EV3) {
		/* make sure we include "cccsp/oil_ev3_lib.c" */
		nlibs++;
	}
	...
	nlibs = 0;
	if (src) {
		tmp[nlibs++] = string_dup ("cccsp/oil_cccsp_lib.c");
	} else {
		tmp[nlibs++] = string_dup ("cccsp/oil_cccsp_lib.o");
	}

	if (cccsp_get_subtarget () == CCCSP_SUBTARGET_EV3) {
		if (src) {
			tmp[nlibs++] = string_dup ("cccsp/oil_ev3_lib.c");
		} else {
			tmp[nlibs++] = string_dup ("cccsp/oil_ev3_lib.o");
		}
	}
...
/*{{{  int oil_declify_listtodecllist (tnode_t **listptr, oil_declify_t *gdl)*/
/*
...
int oil_declify_listtodecllist (tnode_t **listptr, oil_declify_t *gdl)
{
	...
	for (i=0; (i<nitems) && (items[i]->tag == oil.tag_VARDECL); i++);
	for (j=i; (j<nitems) && (items[j]->tag != oil.tag_VARDECL); j++);

	/* note: i is index of first non-decl item, j index of next decl-item or EOL */

#if 0
fprintf (stderr, "oil_declify_listtodecllist(): i=%d, j=%d, nitems=%d\n", i, j, nitems);
#endif
	if (i > 0) {
		...
		tnode_t *vdblock = tnode_createfrom (oil.tag_DECLBLOCK, list, decllist, instlist);
		int k;
		...
		*listptr = vdblock;
		...
		while ((j < nitems) && (items[j]->tag == oil.tag_VARDECL)) {
	...
	if (nextptr) {
		...
		oil_declify_subtree (nextptr, gdl);
	}

	return 0;
}
/*}}}*/
/*{{{  int oil_declify_listtodecllist_single (tnode_t **listptr, oil_declify_t *gdl)*/
/*
...
#if 0
fprintf (stderr, "oil_declify_listtodecllist_single(): nitems=%d\n", nitems);
#endif
	...
	*listptr = newlist;
	...
	oil_declify_subtree (listptr, gdl);

	return 0;
}
/*}}}*/
/*{{{  int oil_autoseq_listtoseqlist (tnode_t **listptr, oil_autoseq_t *gas)*/
/*
...
int oil_autoseq_listtoseqlist (tnode_t **listptr, oil_autoseq_t *gas)
{
	...
	if (nitems > 0) {
		*listptr = tnode_createfrom (oil.tag_SEQ, lst, NULL, lst);
	}
...
/*{{{  int oil_declify_subtree (tnode_t **tptr, oil_declify_t *gdl)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_declify_subtree(): NULL tree-pointer");
		return 1;
	} else if (!gdl) {
		nocc_serious ("oil_declify_subtree(): NULL declify structure");
		return 1;
	} else if (!*tptr) {
		return 0;
	} else {
...
/*}}}*/
/*{{{  int oil_autoseq_subtree (tnode_t **tptr, oil_autoseq_t *gas)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_autoseq_subtree(): NULL tree-pointer");
		return 1;
	} else if (!gas) {
		nocc_serious ("oil_autoseq_subtree(): NULL autoseq structure");
		return 1;
	} else if (!*tptr) {
		return 0;
	} else {
...
/*}}}*/
/*{{{  int oil_flattenseq_subtree (tnode_t **tptr)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_flattenseq_subtree(): NULL tree-pointer");
		return 1;
	} else if (!*tptr) {
		return 0;
	} else {
...
/*}}}*/
/*{{{  int oil_postscope_subtree (tnode_t **tptr)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_postscope_subtree(): NULL tree-pointer");
		return 1;
	} else if (!*tptr) {
		return 0;
	} else {
...
{
	if (!tptr) {
		nocc_serious ("oil_fetrans1_subtree(): NULL tree-pointer");
		fe1->error++;
		return 1;
	} else if (!*tptr) {
		return 0;
...
#if 0
fhandle_printf (FHAN_STDERR, "oil_fetrans1_subtree(): on [%s:%s]\n", (*tptr)->tag->ndef->name, (*tptr)->tag->name);
#endif
...
/*}}}*/
/*{{{  int oil_fetrans1_subtree_newtemps (tnode_t **tptr, oil_fetrans1_t *fe1)*/
/*
...
/*}}}*/
/*{{{  int oil_fetrans15_subtree (tnode_t **tptr, oil_fetrans15_t *fe15)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_fetrans15_subtree(): NULL tree-pointer");
		fe15->error++;
		return 1;
	} else if (!*tptr) {
		return 0;
...
/*}}}*/
/*{{{  int oil_fetrans2_subtree (tnode_t **tptr, oil_fetrans2_t *fe2)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_fetrans2_subtree(): NULL tree-pointer");
		fe2->error++;
		return 1;
	} else if (!*tptr) {
		return 0;
...
/*}}}*/
/*{{{  int oil_fetrans3_subtree (tnode_t **tptr, oil_fetrans3_t *fe3)*/
/*
...
{
	if (!tptr) {
		nocc_serious ("oil_fetrans3_subtree(): NULL tree-pointer");
		fe3->error++;
		return 1;
	} else if (!*tptr) {
		return 0;
...
tnode_t *oil_fetrans1_maketemp (ntdef_t *tag, tnode_t *org, tnode_t *type, tnode_t *init, oil_fetrans1_t *fe1)
{
	char *xname = oil_maketempname (org);
	tnode_t *ndecl, *nname;
	name_t *dname;

#if 0
fhandle_printf (FHAN_STDERR, "oil_fetrans1_maketemp(): here!\n");
#endif
	if (!fe1->decllist) {
		tnode_t *dblk, *dilist;

		if (!fe1->inspoint) {
			nocc_internal ("oil_fetrans1_maketemp(): no insert-point!");
		}
		dilist = parser_newlistnode (SLOCI);
		dblk = tnode_createfrom (oil.tag_DECLBLOCK, org, dilist, *fe1->inspoint);
...
fhandle_printf (FHAN_STDERR, "before insert, *fe1->inspoint = [%s:%s]\n", (*fe1->inspoint)->tag->name, (*fe1->inspoint)->tag->ndef->name);
	...
	ndecl = tnode_createfrom (oil.tag_VARDECL, org, nname, type, init);
...
/*}}}*/
/*{{{  char *oil_maketempname (tnode_t *org)*/
/*
...
/*}}}*/
/*{{{  int oil_chantype_setinout (tnode_t *chantype, int marked_in, int marked_out)*/
/*
...
/*}}}*/
/*{{{  int oil_chantype_getinout (tnode_t *chantype, int *marked_in, int *marked_out)*/
/*
...
/*}}}*/
/*{{{  static copycontrol_e oil_copyctl_aliasnames (tnode_t *node)*/
/*
...
{
	if (!node) {
		nocc_internal ("oil_copyctl_aliasnames(): called with NULL node!");
		return COPY_ALIAS;
	}
	if (node->tag->ndef == oil.node_NAMENODE) {
		return COPY_ALIAS;
	}
	return (COPY_SUBS | COPY_HOOKS | COPY_CHOOKS);
}
/*}}}*/
/*{{{  tnode_t *oil_copytree (tnode_t *tree)*/
/*
 *	copies a parse tree fragment, preserving (aliasing) oil:namenode nodes
 *	returns new tree on success, NULL on failure (or copy of a null tree)
 */
tnode_t *oil_copytree (tnode_t *tree)
{
	tnode_t *copy = tnode_copyoraliastree (tree, oil_copyctl_aliasnames);
...
static tnode_t *oil_make_tlp (tnode_t *userfcn)
{
	...
	tnode_t *sd_call;
	oil_fcndefhook_t *fdh, *userfdh;
	tnode_t *ucallparams;

	userfdh = (oil_fcndefhook_t *)tnode_nthhookof (userfcn, 0);
	if (!userfdh->pfcndef) {
		nocc_error ("oil_make_tlp(): user process has no PFCNDEF equivalent..");
		return NULL;
	}
	...
	if (parser_islistnode (params)) {
		...
		for (i=0; i<npitems; i++) {
			...
			if (pitems[i]->tag != oil.tag_FPARAM) {
				nocc_error ("oil_make_tlp(): parameter is not FPARAM, got [%s]", pitems[i]->tag->name);
				return NULL;
			}

#if 0
fprintf (stderr, "oil_make_tlp(): looking at parameter %d, FPARAM of:\n", i);
tnode_dumptree (pitems[i], 1, FHAN_STDERR);
			...
			default:
				nocc_error ("oil_make_tlp(): could not guess top-level parameter usage (%d)", i);
				return NULL;
			case 1:
				if (has_keyboard >= 0) {
					nocc_error ("oil_make_tlp(): confused, two keyboard channels? (%d)", i);
					return NULL;
				}
				has_keyboard = i;
				break;
			case 2:
				if (has_screen >= 0) {
					if (has_error >= 0) {
						nocc_error ("oil_make_tlp(): confused, two screen channels? (%d)", i);
						return NULL;
					}
					has_error = i;
				} else {
			...
			case 3:
				if (has_error >= 0) {
					nocc_error ("oil_make_tlp(): confused, two error channels? (%d)", i);
					return NULL;
				}
				has_error = i;
				break;
		...
		nocc_error ("oil_make_tlp(): no top-level parameters, at all");
		return NULL;
	}
#if 0
fhandle_printf (FHAN_STDERR, "oil_make_tlp(): here, has_keyboard=%d, has_screen=%d, has_error=%d\n", has_keyboard, has_screen, has_error);
#endif

	/*{{{  create empty-parameter list and name*/
	tlparams = parser_newlistnode (SLOCI);
	tlpname = name_addname ("oil_main", NULL, tlparams, NULL);
	tlpnamenode = tnode_create (oil.tag_NPFCNDEF, SLOCI, tlpname);
	...
	tlpbodypar = tnode_create (oil.tag_PAR, SLOCI, NULL, tlpbodypitems);
	tlpbodysitems = parser_newlistnode (SLOCI);
	tlpbodyseq = tnode_create (oil.tag_SEQ, SLOCI, NULL, tlpbodysitems);
	parser_addtolist (tlpbodysitems, tlpbodypar);
	parser_addtolist (tlpbodysitems, tnode_create (oil.tag_SHUTDOWN, SLOCI));
	tlpbody = tnode_create (oil.tag_DECLBLOCK, SLOCI, tlpbodydlist, tlpbodyseq);
	fdh = oil_newfcndefhook ();
	fdh->lexlevel = 0;
	fdh->ispublic = 1;
	fdh->istoplevel = 1;
	fdh->ispar = 0;
	fdh->pfcndef = NULL;
	tlpdef = tnode_create (oil.tag_PFCNDEF, SLOCI, tlpnamenode, tlparams, tlpbody, NULL, fdh);
		...
		for (j=0; j<3; j++) {
			if (cids[j] >= 0) {
				...
				if (!utype) {
					...
					return NULL;
				} else if (utype->tag != oil.tag_CHAN) {
				...
				ctype = oil_newchantype (oil.tag_CHAN, utype, prot);
				cname = name_addname (cnamestrs[j], NULL, ctype, NULL);
				cnamenode = tnode_create (oil.tag_NDECL, SLOCI, cname);
				...
				*pptr = cnamenode;
				...
				cdecl = tnode_create (oil.tag_VARDECL, SLOCI, cnamenode, ctype, NULL);
	...
	/*{{{  create instance of the user-process*/
	{
		...
		inst = tnode_create (oil.tag_PPINSTANCE, SLOCI, tnode_nthsubof (userfdh->pfcndef, 0), NULL, ucallparams);
	...
	{
		int cids[3] = {has_keyboard, has_screen, has_error};
		char *cprocstrs[3] = {"oil_keyboard_process", "oil_screen_process", "oil_error_process"};
		...
		for (j=0; j<3; j++) {
			if (cids[j] >= 0) {
				...
				tnode_t *ndecl, *nname;
				oil_fcndefhook_t *fdh;
				tnode_t *inst, *iparms;

				if (!fcname) {
					nocc_error ("oil_make_tlp(): failed to find a process called \"%s\"", cprocstrs[j]);
					return NULL;
				}
				ndecl = NameDeclOf (fcname);
				nname = NameNodeOf (fcname);

				if (ndecl->tag != oil.tag_FCNDEF) {
					nocc_error ("oil_make_tlp(): looked up \"%s\" but not a function [%s:%s]",
							cprocstrs[j], ndecl->tag->ndef->name, ndecl->tag->name);
					return NULL;
				}

				fdh = (oil_fcndefhook_t *)tnode_nthhookof (ndecl, 0);
				if (!fdh) {
					nocc_error ("oil_make_tlp(): function \"%s\" has no function-hook", cprocstrs[j]);
					return NULL;
				} else if (!fdh->pfcndef) {
					nocc_error ("oil_make_tlp(): function \"%s\" has no pfcn attached", cprocstrs[j]);
					return NULL;
				}
				...
				inst = tnode_create (oil.tag_PPINSTANCE, SLOCI, nname, NULL, iparms);
				parser_addtolist (tlpbodypitems, inst);

#if 0
fhandle_printf (FHAN_STDERR, "oil_make_tlp(): looked up \"%s\", found:\n", cprocstrs[j]);
if (fcname) {
...
/*}}}*/
/*{{{  static int oil_insert_tlp_shutdown (tnode_t *fcndef)*/
/*
...
static int oil_insert_tlp_shutdown (tnode_t *fcndef)
{
	...
	if (fcndef->tag != oil.tag_FCNDEF) {
		nocc_error ("oil_insert_tlp_shutdown(): top-level function not FCNDEF, got [%s:%s]", fcndef->tag->ndef->name, fcndef->tag->name);
		return -1;
	}
	body = tnode_nthsubof (fcndef, 2);
	if (body->tag == oil.tag_SEQ) {
		...
		newseq = tnode_create (oil.tag_SEQ, SLOCI, NULL, slist);
		parser_addtolist (slist, body);
		tnode_setnthsub (fcndef, 2, newseq);
		body = newseq;
	}
	parser_addtolist (slist, tnode_create (oil.tag_SHUTDOWN, SLOCI));
...
static int declify_cpass (tnode_t **treeptr)
{
	oil_declify_t *gdl = (oil_declify_t *)smalloc (sizeof (oil_declify_t));
	int err = 0;

	gdl->errcount = 0;
	oil_declify_subtree (treeptr, gdl);
	err = gdl->errcount;
	sfree (gdl);
...
static int autoseq_cpass (tnode_t **treeptr)
{
	oil_autoseq_t *gas = (oil_autoseq_t *)smalloc (sizeof (oil_autoseq_t));
	int err = 0;

	gas->errcount = 0;
	oil_autoseq_subtree (treeptr, gas);
	err = gas->errcount;
	sfree (gas);
...
static int flattenseq_cpass (tnode_t **treeptr)
{
	oil_flattenseq_subtree (treeptr);
	return 0;
}
/*}}}*/
...
static int postscope_cpass (tnode_t **treeptr)
{
	...
	if (!compopts.notmainmodule) {
		...
		for (i=nitems-1; i>=0; i--) {
			if (items[i]->tag == oil.tag_FCNDEF) {
				oil_fcndefhook_t *fdh = (oil_fcndefhook_t *)tnode_nthhookof (items[i], 0);
...
static int fetrans1_cpass (tnode_t **treeptr)
{
	oil_fetrans1_t *fe1 = oil_newfetrans1 ();
	int err;

	oil_fetrans1_subtree (treeptr, fe1);

	err = fe1->error;
	oil_freefetrans1 (fe1);
...
static int fetrans15_cpass (tnode_t **treeptr)
{
	oil_fetrans15_t *fe15 = oil_newfetrans15 ();
	int err;

	oil_fetrans15_subtree (treeptr, fe15);

	err = fe15->error;
	oil_freefetrans15 (fe15);
...
static int fetrans2_cpass (tnode_t **treeptr)
{
	oil_fetrans2_t *fe2 = oil_newfetrans2 ();
	int err;
	...
	oil_fetrans2_subtree (treeptr, fe2);

	err = fe2->error;
	oil_freefetrans2 (fe2);

	if (!err) {
		/* before we finish here, insert MAPINIT for language-specific initialisation in namemap */
		tnode_t *minode = tnode_create (oil.tag_MAPINIT, NULL);
...
static int fetrans3_cpass (tnode_t **treeptr)
{
	oil_fetrans3_t *fe3 = oil_newfetrans3 ();
	int err;

	oil_fetrans3_subtree (treeptr, fe3);

	err = fe3->error;
	oil_freefetrans3 (fe3);
...
static int fetrans4_cpass (tnode_t **treeptr)
{
	...
	if (parser_islistnode (tnode_nthsubof (tlfcn, 1)) && !parser_countlist (tnode_nthsubof (tlfcn, 1))) {
		...
		if (oil_insert_tlp_shutdown (tlfcn)) {
...
/*}}}*/
/*{{{  static tnode_t *oil_parsemoduledef (lexfile_t *lf)*/
/*
	...
	if (tok->type == INDENT) {
		...
		while (tok->type != OUTDENT) {
			...
			if ((tok->type == COMMENT) || (tok->type == NEWLINE)) {
				/* consume quietly */
			} else if (lexer_tokmatch (oil.tok_ATSIGN, tok)) {
				lexer_freetoken (tok);
				tok = lexer_nexttoken (lf);
...
#if 0
fprintf (stderr, "oil_parsemoduledef(): FIXME: handle version! [%s]\n", vstr);
#endif
					sfree (vstr);
...
#if 0
fprintf (stderr, "oil_parsemoduledef(): FIXME: handle API [%d]\n", vapi);
#endif
					/*}}}*/
...
/*}}}*/
/*{{{  static tnode_t *oil_parsemodusedef (lexfile_t *lf)*/
/*
...
static int oil_parser_init (lexfile_t *lf)
{
	if (!oil_priv) {
		...
		oil_priv = oil_newoilparse ();

		if (compopts.verbose) {
			nocc_message ("initialising oil parser..");
		}

		memset ((void *)&oil, 0, sizeof (oil));

		oil_priv->langdefs = langdef_readdefs ("oil.ldef");
		if (!oil_priv->langdefs) {
			nocc_error ("oil_parser_init(): failed to load language definitions!");
			return 1;
		}

		/*{{{  register some particular tokens (for later comparison)*/
		oil.tok_ATSIGN = lexer_newtoken (SYMBOL, "@");
		oil.tok_STRING = lexer_newtoken (STRING, NULL);
		oil.tok_PUBLIC = lexer_newtoken (KEYWORD, "public");
		oil.tok_SEQUENCE = lexer_newtoken (SYMBOL, ";");

		/*}}}*/
		/*{{{  register some general reduction functions*/
		fcnlib_addfcn ("oil_nametoken_to_hook", (void *)oil_nametoken_to_hook, 1, 1);
		fcnlib_addfcn ("oil_reduce_checkfixio", (void *)oil_reduce_checkfixio, 0, 3);
		...
		opts_add ("stop-declify", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after declify pass");
		if (nocc_addcompilerpass ("declify", INTERNAL_ORIGIN, "pre-scope", 0, (int (*)(void *))declify_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"declify\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-auto-sequence", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after auto-sequence pass");
		if (nocc_addcompilerpass ("auto-sequence", INTERNAL_ORIGIN, "declify", 0, (int (*)(void *))autoseq_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"auto-sequence\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-flattenseq", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after flatten-sequence pass");
		if (nocc_addcompilerpass ("flattenseq", INTERNAL_ORIGIN, "auto-sequence", 0, (int (*)(void *))flattenseq_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"flattenseq\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-postscope", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after post-scope pass");
		...
		if (nocc_addcompilerpass ("fetrans1", INTERNAL_ORIGIN, "fetrans", 0, (int (*)(void *))fetrans1_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans1\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-fetrans15", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after fetrans15 pass");
		if (nocc_addcompilerpass ("fetrans15", INTERNAL_ORIGIN, "fetrans1", 0, (int (*)(void *))fetrans15_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans15\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-fetrans2", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after fetrans2 pass");
		if (nocc_addcompilerpass ("fetrans2", INTERNAL_ORIGIN, "fetrans15", 0, (int (*)(void *))fetrans2_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans2\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-fetrans3", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after fetrans3 pass");
		if (nocc_addcompilerpass ("fetrans3", INTERNAL_ORIGIN, "fetrans2", 0, (int (*)(void *))fetrans3_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans3\" compiler pass");
			return 1;
		}

		stopat = nocc_laststopat() + 1;
		opts_add ("stop-fetrans4", '\0', oil_opthandler_stopat, (void *)stopat, "1stop after fetrans4 pass");
		if (nocc_addcompilerpass ("fetrans4", INTERNAL_ORIGIN, "fetrans3", 0, (int (*)(void *))fetrans4_cpass, CPASS_TREEPTR, stopat, NULL)) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans4\" compiler pass");
			return 1;
		}
		...
		if (tnode_newcompop ("autoseq", COPS_INVALID, 2, INTERNAL_ORIGIN) < 0) {
			nocc_serious ("oil_parser_init(): failed to add \"autoseq\" compiler operation");
			return 1;
		}
		if (tnode_newcompop ("flattenseq", COPS_INVALID, 1, INTERNAL_ORIGIN) < 0) {
			nocc_serious ("oil_parser_init(): failed to add \"flattenseq\" compiler operation");
			return 1;
		}
		...
		if (tnode_newcompop ("fetrans15", COPS_INVALID, 2, INTERNAL_ORIGIN) < 0) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans15\" compiler operation");
			return 1;
		}
		if (tnode_newcompop ("fetrans2", COPS_INVALID, 2, INTERNAL_ORIGIN) < 0) {
			nocc_serious ("oil_parser_init(): failed to add \"fetrans2\" compiler operation");
			return 1;
		}
		...
		tnode_newlangop ("chantype_setinout", LOPS_INVALID, 3, origin_langparser (&oil_parser));
		tnode_newlangop ("chantype_getinout", LOPS_INVALID, 3, origin_langparser (&oil_parser));

		/*}}}*/
		/*{{{  initialise!*/
		if (feunit_do_init_tokens (0, oil_priv->langdefs, origin_langparser (&oil_parser))) {
			nocc_error ("oil_parser_init(): failed to initialise tokens");
			return 1;
		}
		if (feunit_do_init_nodes (feunit_set, 1, oil_priv->langdefs, origin_langparser (&oil_parser))) {
			nocc_error ("oil_parser_init(): failed to initialise nodes");
			return 1;
		}
		if (feunit_do_reg_reducers (feunit_set, 0, oil_priv->langdefs)) {
			nocc_error ("oil_parser_init(): failed to register reducers");
			return 1;
		}
		if (feunit_do_init_dfatrans (feunit_set, 1, oil_priv->langdefs, &oil_parser, 1)) {
			nocc_error ("oil_parser_init(): failed to initialise DFAs");
			return 1;
		}
		if (feunit_do_post_setup (feunit_set, 1, oil_priv->langdefs)) {
			nocc_error ("oil_parser_init(): failed to post-setup");
			return 1;
		}
		if (langdef_treecheck_setup (oil_priv->langdefs)) {
			nocc_serious ("oil_parser_init(): failed to initialise tree-checking!");
		}

		oil_priv->inode = dfa_lookupbyname ("oil:decl");
		if (!oil_priv->inode) {
			nocc_error ("oil_parser_init(): could not find oil:decl!");
			return 1;
		}

		if (oil_udo_init ()) {
			nocc_error ("oil_parser_init(): failed to initialise user-defined operators!");
			return 1;
		}
		...
		oil.tok_INTERLEAVE = lexer_newtoken (SYMBOL, "|||");
...
/*{{{  static tnode_t *oil_parse_preproc (lexfile_t *lf)*/
/*
...
static tnode_t *oil_parse_preproc (lexfile_t *lf)
{
	...
	tok = lexer_nexttoken (lf);
	if (!lexer_tokmatch (oil.tok_ATSIGN, tok)) {
	...
	if (lexer_tokmatchlitstr (tok, "include")) {
		...
		tree = tnode_create (oil.tag_PPCOMMENT, SLOCN (lf), oil_makestringlit (oil_newprimtype (oil.tag_STRING, NULL, 0), NULL, str));
		sfree (str);

#if 0
fprintf (stderr, "oil_parser_preproc(): here!  created comment node:\n");
...
#if 0
fprintf (stderr, "oil_parse_preproc(): here!  token is:\n");
...
/*}}}*/
/*{{{  static int oil_skiptoeol (lexfile_t *lf, int skipindent)*/
/*
...
/*}}}*/
/*{{{  static int oil_skiptooutdent (lexfile_t *lf)*/
/*
...
static tnode_t *oil_parser_parseproc (lexfile_t *lf)
{
	...
	tree = dfa_walk ("oil:procstart", 0, lf);
	if (!tree) {
	...
	if (tnflags & TNF_LONGPROC) {
		...
		if (ntflags & NTF_INDENTED_PROC_LIST) {
			...
			tnode_t *body = oil_indented_declorproc_list (lf);
			...
			tnode_t *body = oil_indented_dguard_list (lf);

			tnode_setnthsub (tree, 1, body);
			/*}}}*/
		} else if (ntflags & NTF_INDENTED_EXPR_LIST) {
			/*{{{  long process, parse list of indented expressions and processes into subnode 1*/
			tnode_t *body = oil_indented_exprproc_list (lf);

			tnode_setnthsub (tree, 1, body);
			/*}}}*/
		} else {
			tnode_warning (tree, "oil_parser_parseproc(): unhandled LONGPROC [%s]", tree->tag->name);
		}
		/*}}}*/
		...
		if (ntflags & NTF_INDENTED_TCASE_LIST) {
			...
			tnode_t *body = oil_indented_tcase_list (lf);

			tnode_setnthsub (tree, 1, body);
			/*}}}*/
		} else {
			tnode_warning (tree, "oil_parser_parseproc(): unhandled LONGACTION [%s]", tree->tag->name);
		}
		/*}}}*/
	}
...
static tnode_t *oil_declorproc (lexfile_t *lf)
{
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_declorproc(): parsing declaration or process by test at %s:%d", lf->fnptr, lf->lineno);
	}
	...
	tree = dfa_walk ("oil:testfordecl", 0, lf);
	if (!tree) {
		...
		tnode_free (tree);
		tree = oil_parser_parsedef (lf);
	} else if (tree->tag == testfalsetag) {
		/* definitely a process */
		tnode_free (tree);
		tree = oil_parser_parseproc (lf);
		isproc = 1;
	} else {
#if 0
fhandle_printf (FHAN_STDERR, "oil_declorproc(): about to fail, but testtruetag == %p\n", testtruetag);
#endif
		nocc_serious ("oil_declorproc(): oil_testfordecl DFA returned:");
		tnode_dumptree (tree, 1, FHAN_STDERR);
		tnode_free (tree);
		tree = NULL;
	}
	...
	case SYMBOL:
#if 0
fhandle_printf (FHAN_STDERR, "oil_declorproc(): trailing symbol after isproc=%d, tree at %p\n", isproc, tree);
fhandle_printf (FHAN_STDERR, "oil_declorproc(): INTERLEAVE tok = [%s]\n", lexer_stokenstr (oil.tok_INTERLEAVE));
#endif
		if (isproc && tree && (lexer_tokmatch (oil.tok_SEQUENCE, tok) || lexer_tokmatch (oil.tok_INTERLEAVE, tok))) {
			havemore = 1;
			break;		/* switch */
		} /* else fall through */
	default:
		...
		while (again) {
			...
			case SYMBOL:
				if (!ncnode) {
					...
					if (lexer_tokmatch (oil.tok_SEQUENCE, tok)) {
						ncnode = tnode_create (oil.tag_SEQ, SLOCN (lf), NULL, plist);
						seqnotpar = 1;
					} else if (lexer_tokmatch (oil.tok_INTERLEAVE, tok)) {
						ncnode = tnode_create (oil.tag_PAR, SLOCN (lf), NULL, plist);
						seqnotpar = 0;
					} else {
						parser_error (SLOCN (lf), "expected to find newline or comment [A], but found [%s]", lexer_stokenstr (tok));
						again = 0;
					}
				} else if ((seqnotpar == 1) && !lexer_tokmatch (oil.tok_SEQUENCE, tok)) {
					parser_error (SLOCN (lf), "expected to find sequence operator, but found [%s]", lexer_stokenstr (tok));
					again = 0;
				} else if ((seqnotpar == 0) && !lexer_tokmatch (oil.tok_INTERLEAVE, tok)) {
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_declorproc(): done parsing declaration or process, got (%s:%s)", tree ? tree->tag->name : "(nil)",
...
/*}}}*/
/*{{{  static tnode_t *oil_decllistandguard (lexfile_t *lf)*/
/*
...
static tnode_t *oil_decllistandguard (lexfile_t *lf)
{
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_decllistandguard(): parsing declarations(s) and guard at %s:%d", lf->fnptr, lf->lineno);
	}
	...
	for (;;) {
		...
		if (!tree) {
			...
			nocc_serious ("oil_decllistandguard(): oil_testfordecl DFA returned:");
			tnode_dumptree (tree, 1, FHAN_STDERR);
			tnode_free (tree);
			return NULL;
		}
	...
	tree = dfa_walk ("oil:testforexpr", 0, lf);
	if (!tree) {
		...
		tok = lexer_nexttoken (lf);
		if (!tok) {
			nocc_internal ("oil_decllistandguard(): pop!");
			return NULL;
		}

		if (lexer_tokmatchlitstr (tok, "&")) {
			/* yes -- pre-conditioned guard */
			tree = dfa_walk ("oil:guard", 0, lf);
			if (tree) {
			...
			lexer_pushback (lf, tok);
			tree = dfa_walk ("oil:restofinput", 0, lf);
			if (tree) {
				tnode_setnthsub (tree, 0, lexpr);
			}
		} else if (lexpr->tag == oil.tag_MARKEDIN) {
			/* the '?' got consumed in the expression */
#if 0
fhandle_printf (FHAN_STDERR, "oil_decllistandguard(): looking at guard [MARKEDIN], next tok is '%s', lexpr is:\n", lexer_stokenstr (tok));
tnode_dumptree (lexpr, 1, FHAN_STDERR);
#endif
			lexer_pushback (lf, tok);
			tree = dfa_walk ("oil:restofguard2", 0, lf);
			if (tree) {
				if (tree->tag == oil.tag_GUARD) {
		...
		nocc_serious ("oil_decllistandguard(): oil_testforexpr DFA returned:");
		tnode_dumptree (tree, 1, FHAN_STDERR);
		tnode_free (tree);
		return NULL;
	}
	...
	if (!tree) {
		...
		gproc = oil_indented_declorproc_list (lf);
		tnode_setnthsub (tree, 2, gproc);

		if (decls) {
			tnode_t *tmp = tnode_create (oil.tag_DECLBLOCK, SLOCI, decls, tree);
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_decllistandguard(): done parsing declaration or process, got (%s:%s)", tree ? tree->tag->name : "(nil)",
				tree ? tree->tag->ndef->name : "(nil)");
	}
...
static tnode_t *oil_indented_declorproc_list (lexfile_t *lf)
{
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_indented_declorproc_list(): %s:%d: parsing indented declaration or process list", lf->fnptr, lf->lineno);
	}
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_indented_declorproc_list(): %s:%d: done parsing indented process list (tree at %p)",
				lf->fnptr, lf->lineno, tree);
	}

#if 0
fprintf (stderr, "oil_indented_declorproc_list(): returning:\n");
...
static tnode_t *oil_indented_dguard_list (lexfile_t *lf)
{
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_indented_dguard_list(): %s:%d: parsing indented guard list", lf->fnptr, lf->lineno);
	}
	...
	for (;;) {
		...
		thisone = oil_decllistandguard (lf);
	...
	if (compopts.verbose > 1) {
		nocc_message ("oil_indented_dguard_list(): %s:%d: done parsing indented guard list (tree at %p)",
				lf->fnptr, lf->lineno, tree);
	}
	return tree;
}
/*}}}*/
/*{{{  static tnode_t *oil_indented_name_list (lexfile_t *lf)*/
/*
...
static tnode_t *oil_indented_name_list (lexfile_t *lf)
{
	...
	if (compopts.debugparser) {
		nocc_message ("oil_indented_name_list(): %s:%d: parsing indented name list", lf->fnptr, lf->lineno);
	}
	...
	for (;;) {
		...
		thisone = dfa_walk ("oil:nameandassign", 0, lf);
...
static tnode_t *oil_indented_tcase_list (lexfile_t *lf)
{
	...
	if (compopts.debugparser) {
		nocc_message ("oil_indented_tcase_list(): %s:%d: parsing indented tcase list", lf->fnptr, lf->lineno);
	}
	...
	for (;;) {
		...
		thisone = dfa_walk ("oil:otherdecl", 0, lf);
		if (!thisone) {
		...
		thisproc = oil_indented_declorproc_list (lf);
		if (!thisproc) {
			/* failed to parse, give up */
			break;		/* for() */
		}

		dblk = tnode_create (oil.tag_DECLBLOCK, slocn, thisone, thisproc);
...
static tnode_t *oil_indented_exprproc_list (lexfile_t *lf)
{
	...
	if (compopts.debugparser) {
		nocc_message ("oil_indented_exprproc_list(): %s:%d: parsing indented expression list", lf->fnptr, lf->lineno);
	}
	...
	for (;;) {
		...
		thisone = dfa_walk ("oil:expr", 0, lf);
		if (!thisone) {
			/* failed to parse, give up */
			break;		/* for() */
		}
		...
		condnode = tnode_create (oil.tag_COND, slocn, thisone, thisproc);
...
/*{{{  static tnode_t *oil_parser_parsedef (lexfile_t *lf)*/
/*
 *	parses a single oil definition (e.g. process, type, ...)
...
static tnode_t *oil_parser_parsedef (lexfile_t *lf)
{
	...
	if (compopts.debugparser) {
		nocc_message ("oil_parser_parsedef(): starting parse for single definition..");
	}
	...
	if (lexer_tokmatch (oil.tok_PUBLIC, tok)) {
		lexer_freetoken (tok);
		publictag = 1;
	} else {
		lexer_pushback (lf, tok);
	...
	if (lexer_tokmatch (oil.tok_ATSIGN, tok)) {
		...
		*target = oil_parse_preproc (lf);
		goto skipout;
	} else {
		lexer_pushback (lf, tok);
	}
#if 0
fprintf (stderr, "oil_parser_parsedef(): about to walk for oil:decl\n");
#endif
	thisone = dfa_walk ("oil:decl", 0, lf);
#if 0
fprintf (stderr, "oil_parser_parsedef(): done walking for oil:decl, got 0x%8.8x, (%s,%s)\n", (unsigned int)thisone,
	...
	*target = thisone;
	...
	if (publictag) {
		if (tree && (tree->tag == oil.tag_FCNDEF)) {
			library_markpublic (tree);
		}
	} else if (lf->toplevel && lf->sepcomp && tree && (tree->tag == oil.tag_FCNDEF)) {
		library_markpublic (tree);
	}
...
skipout:
	...
	if (compopts.debugparser) {
		nocc_message ("oil_parser_parsedef(): done parsing single definition (%p).", tree);
	}

	return tree;
}
/*}}}*/
/*{{{  static int oil_parser_parsedeflist (lexfile_t *lf, tnode_t **target)*/
/*
...
{
	if (compopts.debugparser) {
		nocc_message ("oil_parser_parsedeflist(): starting parse of process list from [%s]", lf->fnptr);
	}
	...
	for (;;) {
		...
		thisone = oil_parser_parsedef (lf);
#if 0
fprintf (stderr, "oil_parser_parsedeflist(): sausages");
#endif
		if (!thisone) {
		...
		if (!*target) {
			...
			*target = parser_newlistnode (SLOCN (lf));
			...
			nocc_internal ("oil_parser_parsedeflist(): target is not a list! (%s,%s)", (*target)->tag->name, (*target)->tag->ndef->name);
			return -1;
		}
...
static tnode_t *oil_parser_parse (lexfile_t *lf)
{
	...
	if (compopts.verbose) {
		nocc_message ("oil_parser_parse(): starting parse..");
	}

	i = oil_parser_parsedeflist (lf, &tree);
...
static tnode_t *oil_parser_descparse (lexfile_t *lf)
{
	...
	if (compopts.verbose) {
		nocc_message ("oil_parser_descparse(): parsing descriptor(s)...");
	}

	tree = dfa_walk ("oil:descriptor", 0, lf);
#if 0
fhandle_printf (FHAN_STDERR, "oil_parser_descparse(): got descriptor:\n");
tnode_dumptree (tree, 1, FHAN_STDERR);
#endif
	if (!tree) {
	...
	if (tree && (tree->tag == oil.tag_EXTDECL)) {
		/* just this one */
		return tree;
	}
...
static int oil_parser_prescope (tnode_t **tptr, prescope_t *ps)
{
	...
	if (!ps->hook) {
		oil_prescope_t *gps = (oil_prescope_t *)smalloc (sizeof (oil_prescope_t));
...
{
	if (!ss->langpriv) {
		oil_scope_t *gss = (oil_scope_t *)smalloc (sizeof (oil_scope_t));
		...
		if (compopts.tracescope) {
			nocc_message ("SCOPE: oil_parser_scope: about to mod pre/post tree @%p type [%s:%s]",
...
{
	if (!tc->hook) {
		oil_typecheck_t *gtc = (oil_typecheck_t *)smalloc (sizeof (oil_typecheck_t));
...
/*{{{  static int oil_parser_fetrans (tnode_t **tptr, fetrans_t *fe)*/
/*
 *	does front-end transform for oil.
 *	returns 0 on success, non-zero on failure
 */
static int oil_parser_fetrans (tnode_t **tptr, fetrans_t *fe)
{
	...
	if (!parser_islistnode (*tptr)) {
		nocc_internal ("oil_parser_fetrans(): expected list at top-level, got [%s:%s]", (*tptr)->tag->ndef->name, (*tptr)->tag->name);
		return -1;
	}

	gfe = (oil_fetrans_t *)smalloc (sizeof (oil_fetrans_t));
	gfe->preinslist = NULL;
