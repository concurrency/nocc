...
static int oil_openfile (lexfile_t *lf, lexpriv_t *lp);
static int oil_closefile (lexfile_t *lf, lexpriv_t *lp);
static token_t *oil_nexttoken (lexfile_t *lf, lexpriv_t *lp);
static int oil_getcodeline (lexfile_t *lf, lexpriv_t *lp, char **rbuf);


/*}}}*/
/*{{{  public lexer struct*/
langlexer_t oil_lexer = {
	.langname = "oil",
	.langtag = LANGTAG_OIL,
	.fileexts = {".oil", ".oii", NULL},
	.openfile = oil_openfile,
	.closefile = oil_closefile,
	.nexttoken = oil_nexttoken,
	.getcodeline = oil_getcodeline,
	.freelspecial = NULL,
	.parser = NULL
};
...
/*}}}*/
/*{{{  static int oil_escape_char (lexfile_t *lf, oil_lex_t *lop, char **ptr)*/
/*
...
static int oil_escape_char (lexfile_t *lf, oil_lex_t *lop, char **ptr)
{
	...
	if (**ptr != '\\') {
		lexer_error (lf, "oil_escape_char(): called incorrectly");
		(*ptr)++;
		goto out_error1;
	} else {
		(*ptr)++;
...
/*{{{  static int oil_openfile (lexfile_t *lf, lexpriv_t *lp)*/
/*
 *	called once a oil source file has been opened
 */
static int oil_openfile (lexfile_t *lf, lexpriv_t *lp)
{
	oil_lex_t *lop;

	lop = (oil_lex_t *)smalloc (sizeof (oil_lex_t));
...
/*{{{  static int oil_closefile (lexfile_t *lf, lexpriv_t *lp)*/
/*
 *	called before a oil source file is closed
 */
static int oil_closefile (lexfile_t *lf, lexpriv_t *lp)
{
	oil_lex_t *lop = (oil_lex_t *)(lp->langpriv);

	if (!lop) {
		nocc_internal ("oil_closefile() not open! (%s)", lf->filename);
		return -1;
	}
	sfree (lop);
	lp->langpriv = NULL;
...
#if 0
nocc_message ("oil_nexttoken(): newlineflag, thisindent = %d, xind = %d, DA_CUR (offs) = %d, curindent = %d, scanto = %d", thisindent, xind, DA_CUR (lop->indent_offsets), lop->curindent, lop->scanto_indent);
#endif
...
nocc_message ("INDENT: advancing curindent to %d, scanto = %d", lop->curindent, lop->scanto_indent);
	...
	if (((*ch >= 'a') && (*ch <= 'z')) || ((*ch >= 'A') && (*ch <= 'Z'))) {
		...
		kw = keywords_lookup (tmpstr, (int)(dh - ch), LANGTAG_OIL);
		sfree (tmpstr);
		...
		if (!kw) {
			if (nstart < dh) {
				...
				kw = keywords_lookup (tmpstr, (int)(nstart - ch), LANGTAG_OIL);
				sfree (tmpstr);

#if 0
fprintf (stderr, "oil-lexer: number-ending keyword 0x%8.8x\n", (unsigned int)kw);
#endif
	...
	case '\'':
		/* return this as an integer */
		{
			...
			if (*dh == '\\') {
				/* escape character */
				eschar = oil_escape_char (lf, lop, &dh);
				if (eschar == -255) {
					goto out_error1;
				}
				tok->u.ival = eschar;
...
default_label:
		{
			symbol_t *sym = symbols_match (ch, chlim, LANGTAG_OIL);
