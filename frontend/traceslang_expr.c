...
/*}}}*/
/*{{{  static void traceslang_rawnamenode_hook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)*/
/*
...
static void traceslang_rawnamenode_hook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)
{
	...
	fhandle_printf (stream, "<traceslangrawnamenode value=\"%s\" />\n", hook ? (char *)hook : "(null)");
	return;
}
/*}}}*/
...
/*}}}*/
/*{{{  static void traceslang_litnode_hook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)*/
/*
...
static void traceslang_litnode_hook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)
{
	...
	if (node->tag == traceslang.tag_LITSTR) {
		fhandle_printf (stream, "<traceslanglitnode size=\"%d\" value=\"%s\" />\n", lit ? lit->len : 0, (lit && lit->data) ? lit->data : "(null)");
	} else {
