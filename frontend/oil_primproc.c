/*
 *	oil_primproc.c -- oil primitive processes
 *	Copyright (C) 2010-2016 Fred Barnes, University of Kent <frmb@kent.ac.uk>
 *
...
/*{{{  static void oil_reduce_primproc (dfastate_t *dfast, parsepriv_t *pp, void *rarg)*/
/*
...
static void oil_reduce_primproc (dfastate_t *dfast, parsepriv_t *pp, void *rarg)
{
	...
	if (tok->type == KEYWORD) {
		...
		sfree (ustr);
		if (!tag) {
			parser_error (SLOCN (pp->lf), "unknown primitive process [%s] in oil_reduce_primproc()", tok->u.kw->name);
			goto out_error;
		}
	} else {
		parser_error (SLOCN (pp->lf), "unknown primitive process in oil_reduce_primproc()");
		goto out_error;
	}
...
/*{{{  static int oil_primproc_init_nodes (void)*/
/*
...
static int oil_primproc_init_nodes (void)
{
	...
	tnode_setcompop (cops, "namemap", 2, COMPOPTYPE (oil_namemap_leafnode));
	tnode_setcompop (cops, "codegen", 2, COMPOPTYPE (oil_codegen_leafnode));
	tnd->ops = cops;


	i = -1;
	oil.tag_SKIP = tnode_newnodetag ("SKIP", &i, tnd, NTF_NONE);
	i = -1;
	oil.tag_STOP = tnode_newnodetag ("STOP", &i, tnd, NTF_NONE);
	i = -1;
	oil.tag_SHUTDOWN = tnode_newnodetag ("SHUTDOWN", &i, tnd, NTF_NONE);
	/*}}}*/
...
/*{{{  oil_primproc_feunit (feunit_t)*/
feunit_t oil_primproc_feunit = {
	.init_nodes = oil_primproc_init_nodes,
	.reg_reducers = NULL,
	.init_dfatrans = NULL,
	.post_setup = NULL,
	.ident = "oil-primproc"
