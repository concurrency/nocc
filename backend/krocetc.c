...
static void krocetc_namehook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)
{
	...
	fhandle_printf (stream, "<namehook addr=\"%p\" lexlevel=\"%d\" allocwsh=\"%d\" allocwsl=\"%d\" allocvs=\"%d\" allocms=\"%d\" typesize=\"%d\" indir=\"%d\" wsoffset=\"%d\" vsoffset=\"%d\" msoffset=\"%d\" msshadow=\"%d\" typecat=\"0x%16.16lx\" />\n",
			nh, nh->lexlevel, nh->alloc_wsh, nh->alloc_wsl, nh->alloc_vs, nh->alloc_ms, nh->typesize, nh->indir, nh->ws_offset, nh->vs_offset, nh->ms_offset, nh->ms_shadow, (uint64_t)nh->typecat);
	return;
}
/*}}}*/
...
static void krocetc_blockhook_dumptree (tnode_t *node, void *hook, int indent, fhandle_t *stream)
{
	...
	krocetc_isetindent (stream, indent);
	fhandle_printf (stream, "<blockhook addr=\"%p\" lexlevel=\"%d\" allocws=\"%d\" allocvs=\"%d\" allocms=\"%d\" adjust=\"%d\" wsoffset=\"%d\" entrylab=\"%d\" addstaticlink=\"%d\" addvsp=\"%d\" addmsp=\"%d\" addfbp=\"%d\" addmpp=\"%d\" />\n",
			bh, bh->lexlevel, bh->alloc_ws, bh->alloc_vs, bh->alloc_ms, bh->static_adjust, bh->ws_offset, bh->entrylab, bh->addstaticlink, bh->addvsp, bh->addmsp, bh->addfbp, bh->addmpp);
	return;
}
/*}}}*/
